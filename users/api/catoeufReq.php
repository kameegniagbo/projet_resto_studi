...
function getAllData()
{
	...
	$sql = "SELECT dateVente, nom, prix, datehr FROM ".$table0."  ";
	$sqlTot .= $sql;
	$sqlRec .= $sql;
...
function updateData($xprix, $xnom)
{
	...
	$query0="UPDATE ".$table0." SET prix=".checkEmptyInput($xprix).", dateVente='".$dateVente."', datehr='".$datehr."' WHERE id='".$xnom."' ";

	$query1="UPDATE ".$table1." SET prix=".checkEmptyInput($xprix).", datehr='".$datehr."' WHERE dateVente='".$dateVente."' AND nom='".$xnom."' ";
	
	if(mysqli_query($appDBconn, $query0))
	{
        mysqli_query($appDBconn, $query1);

		$responseX=$xnom. ' : Prix mis a jour avec succes. <br>';
	}
	else
	{
		$responseX=$xnom. ' : <b class="text-danger">Echec de la mise à jour de prix. Veuillez signaler cette erreur ! </b>'. mysqli_error($appDBconn).'<br>';
	}
